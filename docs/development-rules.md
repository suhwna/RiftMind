...
- 프론트엔드는 `대시보드형 SPA`를 기본 전제로 설계한다.
- 메인 사용자 흐름은 `소환사 검색 -> 최근 경기 목록 -> 경기 상세` 순서로 유지한다.
- 고급 기능은 별도 도구처럼 분리하지 않고, 메인 흐름 안에 자연스럽게 녹인다.
- 화면은 정보량이 많더라도 `관리 툴`처럼 보이지 않게 구성한다.
- 필터는 사용자가 직접 값을 입력하는 방식보다 `선택형 칩/토글/드롭다운`을 우선한다.
- 검색/필터 결과는 별도 도구 화면보다 `기존 목록이 좁혀지는 경험`에 가깝게 만든다.
- 내부 식별자(`puuid`, `queueId`, 영문 코드)는 UI에 직접 노출하지 않는다.
- 개발자 관점 설명 문구(`participant 기반`, `색인`, `확장 기능용`)는 사용자 화면에 노출하지 않는다.
- 레이아웃은 `큰 카드 + 명확한 간격 + 제한된 강조색`을 기준으로 단정하게 유지한다.
- 화면 상단은 프로젝트 정체성 중심으로 단순하게 두고, 현재 사용자 정보는 본문 카드 안에서 강조한다.
- 스크롤은 페이지 전체가 과도하게 길어지지 않게 제어하고, 필요한 경우 영역 내부 스크롤을 사용한다.
- 최신 트렌드를 따르더라도 과도한 유리 효과, 과한 애니메이션, 장식성 그라데이션 남용은 피한다.
- 스타일 우선순위는 `보기 좋은 화면`보다 `빠르게 읽히는 화면`, `신기한 구성`보다 `익숙하지만 정제된 구성`이다.
